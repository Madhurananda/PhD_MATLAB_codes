...
function [AN, bmSig, OutPut] = AN_Signal_ReconFromFilterbank(sig_name_mono,norm_option,varargin)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%disp('The simulated BM output')
%disp(bmSig)
%disp('This is the delay vector')
%disp(delayVector)
%disp('instantaneous frequency outputs from the filter')
%disp(instf)
%disp('the filter envelope for each frequency')
%disp(env)
...
%         % Mik's spike encoder
%         %zc_all = simple_spike_gen_multilev3(bmSig,n_channels,cochCFs,fs,length_sig,period_frac,sen_levels,sen_multiplier,min_level_zc);
%         %[zc_all,bmSig,threshold_level] = AN_SpikeGen_mono_MJN(bmSig,n_channels,cochCFs,fs,length_sig,period_frac,sen_levels,sen_multiplier,min_level_zc);
%         [zxstructure,bmSig,threshold_level] = AN_SpikeGen_Mono_MJN(bmSig,n_channels,cochCFs,fs,length_sig,period_frac,sen_levels,sen_multiplier,min_level_zc);
...
new_Name = [sig_name_mono(1:end-4) '_NEW_FromFilterbank'];
...
% an_data_files = [sig_name_mono(1:end-4) '_' num2str(sen_levels) '_' num2str(n_channels) '_AN'];
% 
% 
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% delay_zc_cell=cell(1,n_channels);
% for ch = 1:n_channels
...
% 
% if nargin==2
%     multiplied_zc_cell = AN_Reconstruct_GenerateSignal_eachChannel(an_data_files, delay_zc_cell, channel);
% else
%     multiplied_zc_cell = AN_Reconstruct_GenerateSignal_eachChannel(an_data_files, delay_zc_cell);
% end
% 
% 
% 
...
% final_signal = zeros(n_channels,length_sig);
% h4 = waitbar(0,'Please wait untill the final signal are produced...');
% for ch = 1:n_channels
...
% % plot(data_axis,Final_Signal,'b')
% % The output signal should be saved in appropriate name.
% new_Name = [an_data_files(1:end-7) '_NEW_FromFilterbank'];
% new_Name = [new_Name '.wav'];
% wavwrite(Final_Signal,44100,16,new_Name);
% disp(' ')
% disp(' ')
