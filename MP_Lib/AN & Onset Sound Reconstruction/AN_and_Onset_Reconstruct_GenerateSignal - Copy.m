...
function [multiplied_zc_cell, new_AN_zc_cell, delay_zc_cell_AN, delay_zc_cell_Onset, max_Time_Value_AN, min_Time_Value_AN, max_Time_Value_Onset, min_Time_Value_Onset, num_channels, ...
    length_sig, PeakofSignal, Fs, nbits] = AN_and_Onset_Reconstruct_GenerateSignal(an_data_files, Onset_data_files)
...
% Get the Spike trains according to each CHANNELS for AN dataset.  
new_AN_zc_cell = ANbig(1).data.AN.Spike_Assign_Channels;
% Get the Spike trains according to each CHANNELS for Onset dataset.
new_Onset_zc_cell = Onsetbig(1).data.onsetparams.OnsetSpike_Channel;
% new_Onset_zc_cell = AN_and_Onset_Reconstruct_AssignData_eachChannel(Onset_data_files);
...
% Action should be taken for delay vectors for Onset Spikes
...
disp(['The Lower range as - ' num2str(cochCFs(1)) 'Hz frequency to ' num2str(cochCFs(freq_range(1))) 'Hz frequency'])
disp(['The Middle range as - ' num2str(cochCFs(freq_range(1))) 'Hz frequency to ' num2str(cochCFs(freq_range(2))) 'Hz frequency'])
disp(['The Upper range as - ' num2str(cochCFs(freq_range(2))) 'Hz frequency to ' num2str(cochCFs(50)) 'Hz frequency'])
disp(' ')
...
for ch = 1:num_channels
    ...
    %% for the frequency which is considered as low: we generate normal Auditory Nerve signal
    if ch<=freq_range(1)
...
%                             X=horzcat(temp_x,x);
%                             Y=horzcat(temp_y,y);
                            new_Y=horzcat(zero_array_Y, temp_new_y, new_Y); % 'temp_new_y' is the extra wave
                        end
                    end
                    
                    % Now generate the Amplititude Modulated signal and concate with 'new_Y', but
...
%                             new_y_2=zeros(1,length(x_2));
                        ...
                        len_shouldBe = new_time_value(i)+(1/Fs):(1/Fs):new_time_value(i+1);
%                             start_zero_time=end_cycle_time;
%                             end_zero_time = new_time_value(i+1)-cycle_time;
%                             x_2=start_zero_time+(1/Fs):(1/Fs):end_zero_time;
...
%                             X=horzcat(X,x);
    ...
    %% for the frequency which is considered as middle: we generate Amplitude Modulated Signal
    elseif ch>=(freq_range(1)+1) && ch<=freq_range(2)
        % ----------- Only Onset spikes are to be used here --------
        % -- This is where the delay vector effects can be altered. -- %
        % To deactivate the delay compensation, 'time_value' has to be taken from
        % new_Onset_zc_cell, not from 'delay_zc_cell_Onset'.
        ...
        if (~isempty(x_time_value))
            ...
            if (length(new_time_value)==1)
                disp('only One onset. Investigate .. ')
                disp ('Channel')
                disp(ch)
    ...
    %                     end
                    ...
                    if (i==1 && i~=(length(new_time_value)-1))
                        ...
                        if ((new_time_value(i)-cycle_time)<0)
                            % Then, It is necessary to add some signal at the Beginning.
                            zero_array_x = 0:(1/Fs):new_time_value(i);
                            zero_array_Y = zeros(1,length(zero_array_x));
%                             X=horzcat(zero_array_x,x);
%                             Y=horzcat(zero_array_Y,y);
                            ...
                            % It is necessary to add some zeros at the beginning of each channel signal.
                            zero_array_x = 0:(1/Fs):(new_time_value(i)-cycle_time);
            %                 disp('This length of ZEROS has been added before EXTRA SINE WAVE')
            %                 disp(length(zero_array_x))
                            zero_array_y = zeros(1,length(zero_array_x));
                            %zero_array_Y = zero_array_y.*mult;
...
%                             Y=horzcat(zero_array_y,Y);
                            ...
                            % One extra sine wave should be added at the beggining.
                            temp_x = (new_time_value(i)-cycle_time)+(1/Fs):(1/Fs):new_time_value(i);
            %                 disp('This length has been added for EXTRA SINE WAVE')
            %                 disp(length(temp_x))
                            temp_z=0+((2*pi)/length(temp_x)):(2*pi)/length(temp_x):(2*pi);
                            temp_y = sin(temp_z);
...
%                             Y=horzcat(temp_y,y);
                    ...
                    % Check if there is a big gap or not.
                    if (spike_time_diff>(spike_gap*10))
                        % diff = spike_time_diff-(spike_gap);
                        % diff_a = 0:(1/Fs):spike_time_diff;
                        % Generate the first cycle at the beggining
                        % Find out the starting time of the first cycle.
                        start_cycle_time = new_time_value(i);
                        % Find out the end time of the first cycle.
                        end_cycle_time = new_time_value(i)+spike_gap*5;
                        % This is for the first cycle
                        x_1=start_cycle_time+(1/Fs):(1/Fs):end_cycle_time;
                        a1 = crr_freq;
                        a2 = crr_freq - mod_freq;
...
%                         % Generate some zeros at the middle if there is enough space
%                         start_zero_time=end_cycle_time;
%                         end_zero_time = new_time_value(i+1)-spike_gap;
%                         x_2=start_zero_time+(1/Fs):(1/Fs):end_zero_time;
%                         new_y_2=zeros(1,length(x_2));

                        % Generate second cycle at the end
                        % Find out the starting time of the second cycle.
                        start_cycle_time = new_time_value(i+1)-5*spike_gap;
                        % Find out the end time of the second cycle.
                        end_cycle_time = new_time_value(i+1);
...
%                             X=horzcat(X,x);
    ...
    elseif ch>=freq_range(2) && ch<=num_channels
        % ----------- Only Onset spikes are to be used here --------
        % -- This is where the delay vector effects can be altered. -- %
        % To deactivate the delay compensation, 'time_value' has to be taken from
        % new_Onset_zc_cell, not from 'delay_zc_cell_Onset'.
        ...
        if (new_time_value~=0)
            ...
            % So, each sine wave cycle will take (1/frequency) sec.
            cycle_time=1/channel_frequency;
            % Now the length of the 'new_time_value' is necessary to consider. 
            % If that is empty, then ....
            if (isempty(new_time_value))
                disp('no onset at all. Investigate .. ')
                exit
            % If there is only one spike, then ....
            ...
            % normal reconstruction procedure....    
                ...
                for i = 1:length(new_time_value)-1
                    % What is the differnce between this spike and next spike?
                    temp_length = new_time_value(i+1)-new_time_value(i);
                    adj_cycle_time = cycle_time*10;
                    % If the length between two onsets are bigger than ten
                    % times of the length of each cycle, generate white noise
                    % of length ten times of the length of each cycle.
                    % Otherwise it will be the length between two onsets.
                    len_white_noise = temp_length;
                    if (temp_length>adj_cycle_time)
                        len_white_noise = adj_cycle_time;
                    end
                    x=new_time_value(i)+(1/Fs):(1/Fs):new_time_value(i+1);

                    % So, the length of each white noise cycle will be - 
                    % len = 0:(1/Fs):len_white_noise;
                    c=randn(length(x),1); % Additive white Gaussian noise .
                    % Coefficients
                    a=1;
                    b=[1 1];
                    new_y=filter(b,a,c);
...
%      % end
    ...
    % At the last, add the generated signal to the cell array
    ...
    data = wavread([an_data_files(1:end-13) '.wav']);
    plot(t,data, 'y')
    hold on
    plot(t, new_Y)
    hold on
